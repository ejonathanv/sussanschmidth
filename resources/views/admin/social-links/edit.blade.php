<x-admin-layout>
    <div class="text-left">
        <div class="admin-header">
            <h2 icon-title>
                <i class="ion-social-rss-outline"></i> Edit Social Link
            </h2>
        </div>
        ...
        <form method="POST" action="{{ route('social-links.update', $socialLink) }}">
            @csrf
            @method('PATCH')
            
            <div class="form-group">
                <x-input-label for="name" :value="'Name'" />
                <x-text-input id="name" name="name" type="text" class="form-control" required autofocus :value="old('name', $socialLink->name)" placeholder="Instagram, Facebook, etc..." />
                <x-input-error class="mt-2" :messages="$errors->get('name')" />
            </div>

            <div class="form-group">
                <x-input-label for="url" :value="'URL'" />
                <x-text-input id="url" name="url" type="url" class="form-control" required :value="old('url', $socialLink->url)" placeholder="https://instagram.com/username" />
            ...
            <div class="form-group">
                ...
                <select id="icon" name="icon" class="form-control" required>
                    ...
                    @foreach($availableIcons as $iconClass => $iconName)
                        <option value="{{ $iconClass }}" {{ old('icon', $socialLink->icon) == $iconClass ? 'selected' : '' }}>
                            {{ $iconName }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="number" id="order" name="order" class="form-control" min="0" value="{{ old('order', $socialLink->order) }}" placeholder="0 = last">
            ...
            <div class="form-group">
                <div class="custom-control custom-switch">
                    <input type="checkbox" name="active" class="custom-control-input" id="active" value="1" {{ old('active', $socialLink->active) ? 'checked' : '' }}>
